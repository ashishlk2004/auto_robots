...
def robot_controller_actions(context : LaunchContext):
    ...
    # The initial pose file is resolved relative to the share directory of the
    # package named by 'initial_pose_package'.
    yaml_path = os.path.join(
        get_package_share_directory(context.launch_configurations['initial_pose_package']),
        context.launch_configurations['initial_pose_file'])
    ...
    use_nav2 = context.launch_configurations['use_nav2'].lower() in ('true', '1')
    decon_threshold = int(context.launch_configurations['decon_threshold'])
    red_penalty = float(context.launch_configurations['red_penalty'])
    ...
    for robot_number in range(1, num_robots + 1):
        ...
        controller_params = dict(initial_poses[robot_name])
        controller_params['use_sim_time'] = True
        controller_params['use_nav2'] = use_nav2
        controller_params['decon_threshold'] = decon_threshold
        controller_params['red_penalty'] = red_penalty
        controller_params['num_robots'] = num_robots
...
def generate_launch_description():
    ...
    declare_use_nav2_cmd = DeclareLaunchArgument(
        'use_nav2',
        default_value='True',
        description='Whether to use nav2. The solution provides a map of the '
                    'world (solution/config/map2.yaml) and performs best with '
                    'Nav2; without it a simple built-in navigator is used.'
    )

    # Parameters added by the solution
    declare_decon_threshold_cmd = DeclareLaunchArgument(
        'decon_threshold',
        default_value='1',
        description='Contamination level at which a robot visits the '
                    'decontamination zone (contamination rises by 1 per '
                    'second while towing a red barrel)'
    )

    declare_red_penalty_cmd = DeclareLaunchArgument(
        'red_penalty',
        default_value='0.0',
        description='Additional path cost (metres) applied to red barrels '
                    'when selecting a target; biases robots towards clean '
                    'barrels'
    )
    ...
    # Nav2 parameters, using the solution's tuned copy.
    ...
    ld.add_action(declare_decon_threshold_cmd)
    ld.add_action(declare_red_penalty_cmd)
